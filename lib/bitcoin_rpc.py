...
class BitcoinRPC(object):
    ...
    def _call(self, method, params):
        log.debug("RPC Call: %s" % json.dumps(
            {
                'jsonrpc': '2.0',
                'method': method,
                'params': params,
    ...
    def submitblock(self, block_hex, hash_hex, scrypt_hex):
        # try 5 times? 500 Internal Server Error could mean random error or
        # that TX messages setting is wrong
        attempts = 0
        while True:
            attempts += 1
            if self.has_submitblock is True:
                try:
                    log.debug(
                        "Submitting Block with submitblock: attempt #" +
                        str(attempts))
                    log.debug([block_hex, ])
                    ...
                    if attempts > 4:
                        log.exception(
                            "submitblock failed. Problem Submitting block %s" % str(e))
                ...
                try:
                    log.debug(
                        "Submitting Block with getblocktemplate submit: attempt #"+str(attempts))
                    log.debug([block_hex, ])
                    ...
                    if attempts > 4:
                        log.exception(
                            "getblocktemplate submit failed. Problem Submitting block %s" % str(e))
                        log.exception("Try Enabling TX Messages in config.py!")
                        raise
                    else:
                        continue
            # self.has_submitblock = None; unable to detect submitblock, try
            # both
            else:
                try:
                    ...
                    try:
                        log.exception(
                            "submitblock Failed, does the coind have submitblock?")
                        ...
                        if attempts > 4:
                            log.exception(
                                "submitblock failed. Problem Submitting block %s" % str(e))
                            log.exception(
                                "Try Enabling TX Messages in config.py!")
                            raise
                        else:
                            continue
    ...
    def blockexists(self, hash_hex, scrypt_hex):
        ...
        try:
            ...
            try:
                ...
                log.info("Cannot find block for hash_hex %s or scrypt_hex %s" %
                         hash_hex, scrypt_hex)
                defer.returnValue(False)
        ...
        try:
            log.debug("checking block hash against hash of block height: %s",
                      blockheight)
            ...
            if hash == valid_hash:
                log.debug(
                    "Block confirmed: hash of block matches hash of blockheight")
                defer.returnValue(True)
            else:
                log.debug(
                    "Block invisible: hash of block does not match hash of blockheight")
                defer.returnValue(False)
