...
class CTransaction(object):
    ...
    def __init__(self):
        ...
        if settings.COINDAEMON_Reward == 'POW':
            self.nVersion = 1
            if settings.COINDAEMON_TX is not False:
                self.nVersion = 2
            self.vin = []
            self.vout = []
            self.nLockTime = 0
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            self.sha256 = uint256_from_str(
                SHA256.new(SHA256.new(self.serialize()).digest()).digest()
            )
        return self.sha256
...
class CBlock(object):
    ...
    def __init__(self):
        ...
        # private propery for storage of block hash
    ...
    def serialize(self):
        """
            Serializes block and returns a string representation.
            Can't be used for :meth:`self.block_header` as it contains the
            transactions.
        """
        r = []
    ...
    def block_header(self):
        """
            Searlised block header built from object attrs
    ...
    def block_hash(self):
        """
            Hash of the block header using the requisite POW algo
        """
        if not self._block_hash:
            self._block_hash = uint256_from_str(
                self.pow_hash(self.block_header)
            )
    ...
    def pow_hash(self, s):
        """
            Hashes the block candidate with the requisite POW algorithm

            Args:
                s (str): The string to hash using the POW algorithm.
        ...
        if settings.COINDAEMON_ALGO == 'scrypt':
            return ltc_scrypt.getPoWHash(s)
        elif settings.COINDAEMON_ALGO == 'quark':
            return quark_hash.getPoWHash(s)
        elif settings.COINDAEMON_ALGO == 'scrypt-jane':
            return settings.SCRYPTJANE_NAME.getPoWHash(s)
        elif settings.COINDAEMON_ALGO == 'skein':
            return skeinhash.skeinhash(s)
        elif settings.COINDAEMON_ALGO == 'max':
            return sha3_256(s).digest()
        elif settings.COINDAEMON_ALGO == 'keccak':
            s = sha3.SHA3256()

            # not quite sure why this is specal cased
            s.update(s + str(self.nTime))
            return s.hexdigest()
        else:
    ...
    def is_valid(self):
        """
            Checks if block hash <= target and all transactions contained
        ...
        while len(hashes) > 1:
            ...
            for i in xrange(0, len(hashes), 2):
                ...
                newhashes.append(
                    SHA256.new(
                        SHA256.new(hashes[i] + hashes[i2]).digest()
                    ).digest()
                )
            hashes = newhashes
