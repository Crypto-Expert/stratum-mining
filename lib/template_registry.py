...
class TemplateRegistry(object):
    ...
    def __init__(self, block_template_class, coinbaser, bitcoin_rpc,
        ...
        """
            ...
            Args:
                block_template_class: todo
                coinbaser: todo
                bitcoin_rpc (BitcoinRPC): RPC implementation for connecting to
                    the coin daemon.
                instance_id (int). Used for nonce generation.
                on_template_callback: todo
                on_block_callback: todo
        """
        log.debug("Got to Template Registry")
        ...
        self.extranonce2_size = block_template_class \
            .coinbase_transaction_class.extranonce_size \
    ...
    def get_new_extranonce1(self):
        """
            Generates unique extranonce1 (e.g. for newly subscribed connection)
        """
    ...
    def get_last_broadcast_args(self):
        """
            Returns arguments for mining.notify from last known template.
        """
    ...
    def add_template(self, block, block_height):
        """
            Adds new template to the registry.  It also clean up templates
    ...
    def update_block(self):
        """
            Registry calls the getblocktemplate() RPC and build new block
            template.
        """
    ...
    def diff_to_target(self, difficulty):
        """
            ...
            Args:
                difficulty (float): The difficulty for which to return the
                    hash target.

            Returns:
                float. Target value for given difficulty.
        """
        # diff1 is the hash target for a difficulty of 1

        if settings.COINDAEMON_ALGO == 'scrypt' or 'scrypt-jane':
            diff1 = 0x0000ffff00000000000000000000000000000000000000000000000000000000
        elif settings.COINDAEMON_ALGO == 'quark':
            diff1 = 0x000000ffff000000000000000000000000000000000000000000000000000000
        elif settings.coindaemon_algo in ('max', 'keccak'):
    ...
    def get_job(self, job_id, worker_name, ip=False):
        """
            ...
            Args:
                job_id (str): Id of job to return fetch.
                worker_name (str): Worker name fetching the job (for logging).

            Kwargs:
                ip (str): The ip address of the worker associated with the
                    fetch (for logging).
        """
        try:
            j = self.jobs[job_id]
            ...
            log.info(
                "Job id '%s' not found, worker_name: '%s'" %
                (job_id, worker_name)
            )
    ...
    def submit_share(self, job_id, worker_name, session, extranonce1_bin,
                     extranonce2, ntime, nonce, difficulty, ip=False):
        """
            ...
            Args:
                extranonce1_bin: is binary. No checks performed, it should be
                    from session data
                job_id, extranonce2, ntime, nonce: in hex form sent by the
                    client.
                difficulty: decimal number from session, again no checks
                    performed.
                submitblock_callback: reference to method which receive result
                    of submitblock().
        """
        ...
        block_header_str = ''.join(
            [header_bin[i * 4:i * 4 + 4][::-1] for i in range(0, 20)]
        )

        if settings.COINDAEMON_ALGO == 'scrypt':
            hash_bin = ltc_scrypt.getPoWHash(block_header_str)
        elif settings.COINDAEMON_ALGO == 'scrypt-jane':
            if settings.SCRYPTJANE_NAME == 'vtc_scrypt':
                hash_bin = scryptjane.getPoWHash(block_header_str)
            else:
            ...
            hash_bin = quark_hash.getPoWHash(block_header_str)
        elif settings.COINDAEMON_ALGO == 'max':
            hash_bin = sha3_256(''.join(block_header_str)).digest()[0:33]
        elif settings.COINDAEMON_ALGO == 'skeinhash':
            hash_bin = skeinhash.skeinhash(block_header_str)
        elif settings.COINDAEMON_ALGO == 'keccak':
            s = sha3.sha3_256()
            s.update(block_header_str + str(int(ntime, 16)))
            hash_bin = s.hexdigest()
        else:
            hash_bin = util.doublesha(block_header_str)

        hash_int = util.uint256_from_str(hash_bin)
        scrypt_hash_hex = "%064x" % hash_int
        header_hex = binascii.hexlify(header_bin)

        if settings.COINDAEMON_ALGO in ('scrypt', 'scrypt-jane', 'quark'):
        ...
        if hash_int > target_user:
            raise SubmitException(
                "Share is above target. Hash: %s", scrypt_hash_hex
            )
        ...
        if hash_int <= job.target:
            ...
            # Reverse the header and get the potential block hash
            # (for scrypt only)
            if settings.COINDAEMON_ALGO == 'max':
                block_hash_bin = sha3_256(
                    ''.join([header_bin[i * 4:i * 4 + 4][::-1]
                            for i in range(0, 20)])
                    + str(int(ntime, 16))
                ).hexdigest()
            elif settings.COINDAEMON_ALGO == 'keccak':
            ...
            if not job.is_valid():
                # Should not happen
                log.exception(
                    "FINAL JOB VALIDATION FAILED!" +
                    "(Try enabling/disabling tx messages)"
                )
        ...
        if settings.SOLUTION_BLOCK_HASH:
            if settings.COINDAEMON_ALGO == 'keccak':
                ...
                block_hash_bin = util.doublesha(
                    ''.join([header_bin[i * 4:i * 4 + 4][::-1] for i in
                            range(0, 20)])
                )
                block_hash_hex = block_hash_bin[::-1].encode('hex_codec')
