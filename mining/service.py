...
class MiningService(GenericService):
    ...
    def authorize(self, worker_name, worker_password):
        ...
        if Interfaces.worker_manager.authorize(worker_name, worker_password):
            ...
            if settings.ALLOW_EXTERNAL_DIFFICULTY:
                (is_ext_diff, session['difficulty']
                 ) = Interfaces.worker_manager.get_user_difficulty(worker_name)
                self.connection_ref().rpc(
                    'mining.set_difficulty', [session['difficulty'], ], is_notification=True)
            else:
            ...
            Interfaces.worker_manager.worker_log['authorized'][worker_name] = (
                0, 0, False, session['difficulty'], is_ext_diff, Interfaces.timestamper.time())
            return True
        else:
    ...
    def submit(self, worker_name, work_id, extranonce2, ntime, nonce):
        ...
        (valid, invalid, is_banned, diff, is_ext_diff,
         last_ts) = Interfaces.worker_manager.worker_log['authorized'][worker_name]
        ...
        if is_banned and submit_time - last_ts > settings.WORKER_BAN_TIME:
            ...
            (valid, invalid, is_banned, last_ts) = (0,
                                                    0, is_banned, Interfaces.timestamper.time())
        ...
        log.debug(
            ...
            (worker_name, valid, invalid, is_banned, is_ext_diff, last_ts, percent, work_id, job_id, difficulty))
        if not is_ext_diff:
            Interfaces.share_limiter.submit(
                self.connection_ref, job_id, difficulty, submit_time, worker_name)
        ...
        try:
            ...
            Interfaces.share_manager.on_submit_share(
                worker_name, False, False, difficulty,
    ...
    authorize.params = [(
        'worker_name', 'string', 'Name of the worker, usually in the form of user_login.worker_id.'),
